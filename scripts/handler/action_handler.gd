...
func has(action_: StringName) -> bool:
	return _actions.has(action_)

func press(action_: StringName, has_: bool = false) -> void:
	if not has_ or _actions.has(action_):
...
func is_just_pressed(action_: StringName, has_: bool = false) -> bool:
	if has_ and not _actions.has(action_):
		return false
...
func get_axis(negative_action_: StringName, positive_action_: StringName, has_: bool = false) -> float:
	...
	if not _actions.has(negative_action_):
		if not _actions.has(positive_action_):
			return 0.0
	...
	elif not _actions.has(positive_action_):
